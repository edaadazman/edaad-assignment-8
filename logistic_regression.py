...
def generate_ellipsoid_clusters(distance, n_samples=100, cluster_std=0.5):
    ...
    covariance_matrix = np.array(
        [[cluster_std, cluster_std * 0.8], [cluster_std * 0.8, cluster_std]]
    )

    # Generate the first cluster (class 0)
    X1 = np.random.multivariate_normal(
        mean=[1, 1], cov=covariance_matrix, size=n_samples
    )
    ...
    X2 = np.random.multivariate_normal(
        mean=[1 + distance, 1 + distance], cov=covariance_matrix, size=n_samples
    )
...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        plt.plot(
            x_vals, y_vals, "k--", label="Decision Boundary"
        )  # Dashed black line for boundary
        ...
        for level, alpha in zip(contour_levels, alphas):
            class_1_contour = plt.contourf(
                xx, yy, Z, levels=[level, 1.0], colors=["red"], alpha=alpha
            )  # Fading red for Class 1
            class_0_contour = plt.contourf(
                xx, yy, Z, levels=[0.0, 1 - level], colors=["blue"], alpha=alpha
            )  # Fading blue for Class 0
            if level == 0.7:
                distances = cdist(
                    class_1_contour.collections[0].get_paths()[0].vertices,
        ...
        equation_text = (
            f"{beta0:.2f} + {beta1:.2f} * x1 + {beta2:.2f} * x2 = 0\n"
    ...
    plt.subplot(3, 3, 5)
    plt.plot(
        shift_distances, [-b0 / b2 for b0, b2 in zip(beta0_list, beta2_list)], "co-"
    )
